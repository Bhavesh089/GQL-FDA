...
const handleLambdaResponse = (result) => {
    ...
    if(!payload.status){
     const error = payload.error ?  payload.error.message : payload.message
      console.log(error, 'this is error')
      throw new Error(error);
    }else{
      const {data, statusCode, message} = payload
      console.log(data, statusCode, message, payload, 'this is logs-->')
