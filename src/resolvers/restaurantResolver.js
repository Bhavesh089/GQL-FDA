...
const create = async(_, {restaurants}) => {
  ...
  const getRestaurantsByUserId = async(_,{user_id}) => {
    payload = {user_id}
    const result = await invokeLambda(process.env.GET_RESTAURANTS_BY_USER_LAMBDA_NAME, payload)
    console.log(result, 'this is result')
    const {data, message, statusCode} = await handleLambdaResponse(result); // Call the utility function to handle the response
    console.log(data, message, statusCode, 'Lambda Result');
    return {message, statusCode, restaurants: data.restaurants}; // Return the structured response
...
const getRestaurantsByIds = async(_,{ids}) => {
    const payload = {ids}
    const result = await invokeLambda(process.env.GET_RESTAURANTS_BY_IDS_LAMBDA_NAME, payload)
    console.log(result, 'this is result')
    const {data, message, statusCode} = await handleLambdaResponse(result); // Call the utility function to handle the response
    console.log(data, message, statusCode, 'Lambda Result');
    return {message, statusCode, restaurants: data.restaurants}; // Return the structured response
  ...
  const update = async(_, {restaurant}) => {
    // console.log(args, 'this is args-->')
    const payload = restaurant  
          console.log(payload, '-->')
  
        try {
          result = await invokeLambda(process.env.UPDATE_RESTAURANT_LAMBDA_NAME, payload)
          console.log(result, 'this is result')
          const {statusCode, message, data} = await handleLambdaResponse(result); // Call the utility function to handle the response
          console.log( data, 'Lambda Result');
          return {statusCode, message, restaurant: data.restaurant} // Return the structured response
        } catch (error) {
          console.error('Error invoking Lambda:', error);
          throw new Error(error);
        }
